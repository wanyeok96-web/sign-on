 ...
 *   ADMIN_PASSWORD_지리연수 — (선택) 개인 연수(geo1234) 관리자 암호
 *   SCHOOL_SUFFIXES    — (선택) 허용 접미사 쉼표 구분: 이솔고,하길고,지리연수
...
/** 개인 연수(geo1234) — sheetSuffix와 동일하게 유지 */
...
function ensureWorkshopEvent_(schoolSuffix) {
  if (!isWorkshopSuffix_(schoolSuffix)) return;
  const rows = readSheetObjects_(getSheetEvents_(schoolSuffix));
  const found = rows.some(function (r) {
    return String(r.eventId) === WORKSHOP_EVENT_ID;
  });
  if (found) return;
  const today = Utilities.formatDate(new Date(), "Asia/Seoul", "yyyy-MM-dd");
  appendRow_(getSheetEvents_(schoolSuffix), {
    eventId: WORKSHOP_EVENT_ID,
    title: WORKSHOP_EVENT_TITLE,
    date: today,
    location: "",
    description: WORKSHOP_EVENT_TITLE,
    status: "진행중",
    targetType: "all",
    targetData: "",
    active: "Y",
...
function getWorkshopInfo_(schoolSuffix) {
  if (!isWorkshopSuffix_(schoolSuffix)) {
    throw new Error("워크숍 모드가 아닙니다.");
  }
  ensureWorkshopEvent_(schoolSuffix);
  return {
    eventId: WORKSHOP_EVENT_ID,
    title: WORKSHOP_EVENT_TITLE,
  };
  ...
  try {
    ...
    switch (action) {
      ...
      case "getEvents":
        ...
        break;
      case "getWorkshopInfo":
        data = getWorkshopInfo_(schoolSuffix);
        break;
      case "getAdminEvents":
...
function submitSignature_(payload, schoolSuffix) {
  ...
  const workshop = isWorkshopSuffix_(schoolSuffix);
  let payloadStaffId = String(payload.staffId || "").trim();
  let payloadDept = String(payload.department || "").trim();
  let payloadName = String(payload.name || "").trim();

  if (workshop) {
    payloadDept = normalizeSchoolName_(payloadDept);
    payloadName = String(payloadName || "").trim();
    if (!payloadDept) throw new Error("학교명을 입력해 주세요.");
    if (!payloadName) throw new Error("성함을 입력해 주세요.");
    if (!payloadStaffId) {
      payloadStaffId = "ws_" + payloadDept + "|" + payloadName;
    }
  } else {
...
function provisionSchoolSheets(schoolSuffix) {
  ...
  if (isWorkshopSuffix_(suffix)) {
    ensureWorkshopEvent_(suffix);
  }
